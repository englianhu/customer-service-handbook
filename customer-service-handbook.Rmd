...
author: "[®γσ, Eng Lian Hu](http://englianhu.wordpress.com) <img src='figure/shiroto.jpg'
  width='24'> 白戸則道®"
...
pkgs <- c('devtools', 'plyr', 'dplyr', 'magrittr', 'rmarkdown', 'tidyr', 'knitr', 'rmarkdown', 'lubridate', 'formattable', 'htmltools')
...
dat <- read.csv('data/Case_Level2.csv') %>% tbl_df %>% mutate(Interval = cut(Duration.of.calls, breaks = 1:9 * 2))
    ...
    sidebarLayout(
      sidebarPanel(
        sliderInput("duration", "Number of bins:", 
                    min = min(dat$Duration.of.calls), max = max(dat$Duration.of.calls), 
                    round = TRUE, animate = TRUE, value = round(unique(dat$Duration.of.calls)), 0),
        selectInput("charttype", label = "Chart Type", 
                    choices = c("multiBarChart", "stackedAreaChart", "pieChart"), 
                    selected = "multiBarChart")),
      
      # Show a plot of the generated distribution
      mainPanel(
        p('LOADING... Initial openning webpage might take around 1 minute time for reading data.'),
        tabsetPanel(
          tabPanel('Table', div(id = 'chart1', style = 'display:inline;position:absolute', 
                                DT::dataTableOutput('table'))),
          tabPanel('Graph', showOutput('plot', 'nvd3')))
        )
      )
    )
  )
...
server <- shinyServer(function(input, output) {
  ...
  output$table <- renderDataTable({
    DT::datatable(subset(dat, Duration.of.calls %in% input$duration), caption="Table : Number of words", 
                  escape = FALSE, filter = "top", rownames = FALSE,
                  extensions = list("ColReorder" = NULL, "RowReorder" = NULL,
                                    "Buttons" = NULL, "Responsive" = NULL, 
                                    "FixedColumns" = list(leftColumns = 1)),
                  options = list(
                    dom = 'BRrltpi', autoWidth = TRUE,
  ...
  output$plot <- renderChart2({
    p1 = nPlot(Duration.of.calls ~ Call, group = 'Interval', 
          data = subset(dat, Duration.of.calls %in% input$duration), 
          type = input$charttype)
    p1$chart(color = c('brown', 'blue', '#594c26', 'green', 'yellow'))
    p1
    })
  })
...
## 6.1 Documenting File Creation 
  ...
  - [**shiny**](https://github.com/rstudio/shiny) package version: `r packageVersion('shiny')`
  - File version: 1.0.2
  - File latest updated date: `r Sys.Date()`
  - Author Profile: [Ryo®, Eng Lian Hu](https://beta.rstudioconnect.com/englianhu/ryo-eng/)
...
## 6.2 Speech and Blooper
  ...
  I think of apply `htmltools` and `formattable` but there has no any factorial nor numerical variables inside tables. Here I embed a shiny app but there grouping sounds doesn't working well.
